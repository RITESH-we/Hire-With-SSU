...
function loadVerifiedJobs() {
  ...
  buildSidebarChips();
  applyFilters();
  showAISummary("Displaying 24 verified fresher & junior roles with direct company application links across Cybersecurity, AI/ML, Data Science & CSE. Add your Gemini API Key anytime above to enable live real-time web scraping.");
...
async function loadAllJobs(apiKey) {
  ...
  if (collected.length > 0) {
    ...
    toast('⚠️ Live AI search could not connect. Falling back to 24 verified openings.', 'error');
    loadVerifiedJobs();
  }
}
...
refreshBtn.addEventListener('click', () => {
  ...
  if (k) {
    ...
    loadVerifiedJobs();
    toast('🔄 Refreshed 24 verified job listings! Direct apply active.', 'info');
  }
});
...
if (modalClear) {
  modalClear.addEventListener('click', () => {
    ...
    closeModal();
    loadVerifiedJobs();
    toast('⚡ Switched to Free Mode — 24 verified openings active', 'info');
  });
}
